...
effectiveness-fwd k x y [x]≡[y] =
  ...
  ≡⟨ lemma2 ⟩
    incl [ y ]⟨ k ⟩
  ≡⟨ Incl.incl-quot-map-cong y ⟩
    y
  ∎
  where
    lemma1 : x ≡ incl [ x ]⟨ k ⟩ mod (suc k)
    lemma1 = CMK.sym (incl [ x ]⟨ k ⟩) x (suc k) (Incl.incl-quot-map-cong x)
...
effectiveness-bck k x y x≡ymodk+1 = Incl.injective $ Dist.itself-bck i[x]-i[y]≡0 where
  ...
  i[x]≡i[y]modk+1 =
      incl [ x ]⟨ k ⟩
    ≡⟨ Incl.incl-quot-map-cong x ⟩
      x
    ≡⟨ x≡ymodk+1 ⟩
      y
    ≡⟨ CMK.sym (incl [ y ]⟨ k ⟩) y (suc k) (Incl.incl-quot-map-cong y) ⟩
      incl [ y ]⟨ k ⟩
    ∎
...
split-surjective {k} x = Incl.injective $ Dist.itself-bck i[ix]-ix≡0 where
  ...
  i[ix]-ix≡0 = Divides.divisor-less-than-dividend-fwd i[ix]-ix<k+1 (Incl.incl-quot-map-cong (incl x))
